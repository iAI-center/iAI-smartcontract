...
contract GREENPresale is Ownable, ReentrancyGuard, Pausable {
    using EnumerableSet for EnumerableSet.AddressSet;

    IERC20 public immutable usdtToken;
    IERC20 public immutable greenPresaleToken;

    uint256 public immutable usdtTokenDecimal;
    uint256 public immutable greenTokenTokenDecimal;
    ...
    event WhitelistMaxAmountUpdated(address account, uint256 newMaxAmount);
    ...
    constructor(
        ...
        require(
            _usdtToken != address(0) && _greenPresaleToken != address(0),
            "Invalid token addresses"
        );
        require(_tokenPrice > 0, "Invalid token price");
        require(_endTime > _startTime, "End time must be after start time");
        require(_maxSaleAmount > 0, "Invalid max sale amount");
        require(_revenueReceiver != address(0), "Invalid revenue receiver");

        usdtToken = IERC20(_usdtToken);
        greenPresaleToken = IERC20(_greenPresaleToken);
        usdtTokenDecimal = ERC20(_usdtToken).decimals();
        greenTokenTokenDecimal = ERC20(_greenPresaleToken).decimals();
    ...
    function estimateReceived(
        ...
        uint256 tokenAmount = (usdtAmount * (10 ** greenTokenTokenDecimal)) /
            (tokenPrice);
    ...
    function buyTokens(
        ...
        require(
            greenPresaleToken.balanceOf(address(this)) >= tokenAmount,
        ...
        require(
            greenPresaleToken.transfer(msg.sender, tokenAmount),
    ...
    function withdrawUnsoldPresaleTokens() external onlyOwner {
        ...
        uint256 balance = greenPresaleToken.balanceOf(address(this));
        require(balance > 0, "No tokens to withdraw");
        require(
            greenPresaleToken.transfer(owner(), balance),
            "Token transfer failed"
        );
    ...
    function updateWhitelistMaxAmount(
        ...
        emit WhitelistMaxAmountUpdated(account, newMaxUSDTAmount);
    }
    ...
    function prematureWithdrawPresaleTokens(
        ...
        require(
            greenPresaleToken.balanceOf(address(this)) >= amount,
            "Insufficient token balance"
        );

        require(
            greenPresaleToken.transfer(owner(), amount),
            "Token transfer failed"
        );
