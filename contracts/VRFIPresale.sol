...
contract VRFIPresale is Ownable, ReentrancyGuard, Pausable {
    using EnumerableSet for EnumerableSet.AddressSet;

    IERC20 public immutable usdtToken;
    IERC20 public immutable vrfiPresaleToken;

    uint256 public immutable usdtTokenDecimal;
    uint256 public immutable vrfiTokenTokenDecimal;
    ...
    constructor(
        ...
        require(
            _usdtToken != address(0) && _vrfiPresaleToken != address(0),
        ...
        vrfiPresaleToken = IERC20(_vrfiPresaleToken);
        usdtTokenDecimal = ERC20(_usdtToken).decimals();
        vrfiTokenTokenDecimal = ERC20(_vrfiPresaleToken).decimals();
    ...
    function estimateReceived(
        ...
        uint256 tokenAmount = (usdtAmount * (10 ** vrfiTokenTokenDecimal)) /
            (tokenPrice);
    ...
    function buyTokens(
        ...
        require(
            vrfiPresaleToken.balanceOf(address(this)) >= tokenAmount,
        ...
        require(
            vrfiPresaleToken.transfer(msg.sender, tokenAmount),
    ...
    function withdrawUnsoldPresaleTokens() external onlyOwner {
        ...
        uint256 balance = vrfiPresaleToken.balanceOf(address(this));
        require(balance > 0, "No tokens to withdraw");
        require(
            vrfiPresaleToken.transfer(owner(), balance),
            "Token transfer failed"
        );
    ...
    function prematureWithdrawPresaleTokens(
        ...
        require(
            vrfiPresaleToken.balanceOf(address(this)) >= amount,
            "Insufficient token balance"
        );

        require(
            vrfiPresaleToken.transfer(owner(), amount),
            "Token transfer failed"
        );
